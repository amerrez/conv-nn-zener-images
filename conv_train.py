...
if __name__ == "__main__":
    ...
    loss = tf.reduce_mean(cost_func)
    weights = tf.trainable_variables() # all vars of your graph

    if cost == cost_mode[0]: # cross entropy
        regularized_loss = loss  # this is not really regulaized

    elif cost == cost_mode[1]: #cross entropy with L1 regularization
        l1_regularizer = tf.contrib.layers.l1_regularizer(scale=0.005, scope=None)
        regularization_penalty = tf.contrib.layers.apply_regularization(l1_regularizer, weights)
        regularized_loss = loss + regularization_penalty # this loss needs to be minimized

    elif cost == cost_mode[2]:
        regularization_penalty = tf.nn.l2_loss(weights)
        regularized_loss = loss + regularization_penalty # this loss needs to be minimized

    optimizer = tf.train.GradientDescentOptimizer(epsilon).minimize(regularized_loss)
    session = tf.Session()
    ...
    for i in range(5):  # 5-fold
        ...
        for e in range(max_updates):  # an update is an epoch
            ...
            for b in range(0, len(train_x), batch_size):
                ...
                cost_value = session.run(regularized_loss, feed_dict=feed_data)
